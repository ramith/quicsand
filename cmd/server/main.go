...
func setupHandler(www string, trace bool, logger *zap.SugaredLogger) http.Handler {
    ...
    mux.HandleFunc("/demo/upload", func(w http.ResponseWriter, r *http.Request) {
        if r.Method == http.MethodPost {
            ...
            if err != nil {
                logger.Errorw("Error receiving upload", err)
            }
        }
...
func main() {
    ...
    handler := setupHandler(*www, *trace, logger)
...
func GetCertificatePaths() (string, string) {
    ...
    return path.Join(currentDir, "security", "cert.pem"), path.Join(currentDir, "security", "priv.key")
}
